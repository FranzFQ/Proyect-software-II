...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Simulamos una petición al backend (Loading State)
    setIsLoading(true);
    
    setTimeout(() => {
      setIsLoading(false);
      navigate('/dashboard');
    }, 1500); // Espera 1.5 segundos simulando la conexión
  };
      ...
      <div className="w-full md:w-1/2 bg-[#F8FAFC] flex flex-col justify-center items-center p-8 lg:p-16">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <Button 
              variant="primary" 
              className="w-full py-3 mt-4 text-lg flex justify-center items-center gap-2 disabled:opacity-70"
              disabled={isLoading}
            >
              {isLoading ? (
                <>
