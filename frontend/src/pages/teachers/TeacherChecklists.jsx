...
// ─── Gráfica de puntos — lógica original intacta ──────────────────────────────
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-gray-500 font-bold mb-3">
              {titulo} · <span className="text-gray-400 font-medium">{fecha}</span>
            </p>
        ...
        <div className="mt-6 md:mt-0">
          <div className="flex flex-col items-center bg-gray-50 px-8 py-4 rounded-xl border border-gray-100 shadow-sm">
            <span className={`text-5xl font-black leading-none ${!isNaN(punteoNum) ? colorPunteoText(punteoNum) : 'text-gray-400'}`}>
              {punteo}
            </span>
            ...
            <div className="flex flex-col p-2">
              {criterios.map((nombre, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex items-center gap-4 px-6 py-4 border-b border-gray-50 last:border-b-0 transition hover:bg-gray-50 rounded-lg ${!ev.completado ? 'opacity-55 grayscale' : ''}`}
                  >
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-white border border-gray-200 p-8 rounded-xl shadow-sm h-full">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 min-h-[10rem]">
                {observaciones
                  ? <p className="text-gray-600 text-sm leading-relaxed">{observaciones}</p>
                  : <p className="text-gray-400 text-sm italic">Sin observaciones registradas.</p>
                }
              </div>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col gap-6 min-h-[calc(100vh-4rem)] pb-12">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center shadow-sm">
        ...
        <div className="mt-6 md:mt-0">
          <div className="flex flex-col items-center bg-gray-50 px-8 py-4 rounded-xl border border-gray-100 shadow-sm">
            <span className={`text-5xl font-black leading-none ${!isNaN(promedioNum) ? colorPunteoText(promedioNum) : 'text-[#112240]'}`}>
              {promedio}
            </span>
      ...
      {/* Botón gráfica desplegable — lógica original */}
      {observations.length > 0 && (
        <div>
          <button
            onClick={() => setChartOpen(o => !o)}
            className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 rounded-xl shadow-sm text-[#112240] font-bold text-sm hover:bg-gray-50 transition-all w-full"
          >
          ...
          <div
            ...
            style={{ maxHeight: chartOpen ? '500px' : '0px', opacity: chartOpen ? 1 : 0 }}
          >
            <div className="mt-3">
              {/* Igual que el original: reverse para orden cronológico, slice 5 */}
      ...
      {observations.length === 0 ? (
        <div className="bg-white rounded-xl p-8 text-center text-gray-400 border border-gray-200 shadow-sm flex-1 flex items-center justify-center">
          <p>Este docente no tiene checklists registradas aún.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 content-start">
          {currentItems.map((obs) => {
            ...
            return (
              ...
              >
                {/* Barra superior de color según punteo */}
                ...
                <div className="p-6 pb-2 pt-5">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-right shrink-0">
                      <span className={`text-4xl font-black leading-none ${punteo > 0 ? colorPunteoText(punteo) : 'text-gray-400'}`}>
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mb-2">
                    <div
                      className={`h-1.5 rounded-full ${punteo > 0 ? colorPuntaoBg(punteo) : 'bg-transparent'}`}
                      style={{ width: `${punteo > 0 ? Math.min(punteo * 10, 100) : 0}%` }}
                    />
                  </div>
