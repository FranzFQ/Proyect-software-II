...
import Modal from '../components/common/Modal'; // Reutilizamos nuestro modal
import Button from '../components/common/Button';
import { EyeIcon, TrashIcon, ArrowUpTrayIcon, UserPlusIcon } from '@heroicons/react/24/outline';
...
const Docentes = () => {
  ...
  // Función para ejecutar la eliminación (Simulada en el Frontend)
  const ejecutarEliminacion = () => {
    setDocentes(docentes.filter(doc => doc.id !== docenteAEliminar.id));
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Encabezado y Botones de Acción Superiores */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {/* NUEVOS BOTONES: Cargar y Agregar */}
              ...
              {docentesFiltrados.length === 0 ? (
                ...
                docentesFiltrados.map((doc, index) => (
                  <tr key={doc.id} className={`border-b border-gray-100 hover:bg-gray-50 transition ${index % 2 !== 0 ? 'bg-gray-50/50' : ''}`}>
                    ...
                    <td className="py-4 px-6 text-center">{renderEstado(doc.estado)}</td>
                    <td className="py-4 px-6 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {/* Botón Ver Perfil */}
                        <button 
                          onClick={() => navigate(`/docentes/${doc.id}`)}
                          className="w-9 h-9 border-2 border-url-yellow text-url-yellow rounded-md flex items-center justify-center hover:bg-url-yellow hover:text-white transition-colors"
                          title="Ver perfil"
                        >
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Confirmar Eliminación">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4 text-gray-700">
            ...
            <p>
              ¿Estás seguro de que deseas eliminar al docente <strong className="text-url-blue">{docenteAEliminar?.nombre}</strong> del sistema? Esta acción no se puede deshacer.
            </p>
          </div>
